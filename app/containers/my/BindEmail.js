...
class BindEmail extends Component {
  ...
  enableNext() {
    if (this.isEmpty(this.state.email)) return false;
    return true;
  }
  ...
  render() {
    let email = this.props.user.get('Email') || '未绑定';
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#f2f2f2' }}>
        ...
        <Text style={{ margin: 16, fontSize: 14, color: '#888' }}>{`当前邮箱:${email}`}</Text>
...
export default connect(mapStateToProps, { updateUser })(BindEmail);
