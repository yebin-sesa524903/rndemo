...
export default class RoomPanelPager extends Component{
  constructor(props){
    super(props);
    this.state={}
  }
  ...
  _getTabArray(){
    let oneLabe=this.props.assetType=='Room'?'配电室信息':'配电柜信息';
    let subType=this.props.ownData.get('SubType');
    if (subType===8) {
      oneLabe='楼层信息';
    }else if (subType===70) {
      oneLabe='配电箱信息';
    }
    if(!this.props.showLogTab) return[oneLabe];
    var array = [oneLabe,'维护日志'];
    return array;
  }
  _getTabControl(){
  ...
  _showAlertDialog(){
    let title='删除当前配电柜？';
    if(this.props.assetType=='Room') title='删除当前配电室？';
    let subType=this.props.ownData.get('SubType');
    if (subType===8) {
      title='删除当前楼层？';
    }else if (subType===70) {
  ...
  _showMenu(){
    let lable='编辑配电柜';
    if(this.props.assetType=='Room') lable='编辑配电室';
    let subType=this.props.ownData.get('SubType');
    if (subType===8) {
      lable='删除当前楼层？';
    }else if (subType===70) {
