...
export default class BoardNeedle extends Component {
  ...
  convertAngle = ({ radius, needleRadius, centerSpotRadius, angle, needleAngle }) => {
    ...
    const { x: x2, y: y2 } = paintbrush.drawPoint(radius, centerSpotRadius - 5, angle - needleAngle / 2);
    const { x: x3, y: y3 } = paintbrush.drawPoint(radius, centerSpotRadius - 5, angle + needleAngle / 2);
    ...
    return (
      <G>
        ...
        <Circle
          cx={this.props.radius}
          cy={this.props.radius}
          r={this.props.centerSpotRadius - 8.5}
          strokeWidth={6.5}
          stroke={this.props.needleColor}
          fill="none"
        />

        <Polyline
          points={`${x1},${y1} ${x2},${y2} ${x3},${y3}`}
          fill={this.props.needleColor}
        />
        <Polyline
