...
import {getFilePathByPictureId} from "../../utils/patrolImageUtil";
// import NetworkText from './ticket/NetworkText.js';
// import ImagePicker from './ImagePicker.js';

// var {ImagePickerManager} = NativeModules;

export default class PatrolItemRemarkView extends Component{
  constructor(props){
    super(props);
  ...
  _getImageView() {
    let images = this.props.log.get('Pictures').map((item, index) => {
      if(this.props.offlineMode){
        let imgUri = item.get('uri');
        if (!imgUri) {
          imgUri = item.get('PictureId');
        }
        if (!imgUri) {
          imgUri = require('../../images/building_default/building.png');
        }
  ...
  render() {
    ...
    var lines = 0;
    var content = this.props.log.get('Comment')||'';
...
PatrolItemRemarkView.propTypes = {
